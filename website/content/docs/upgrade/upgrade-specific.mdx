...
(a.k.a. stanzas). Check [HCL](/nomad/docs/job-specification/hcl2) for more details.
...
Volumes](/nomad/docs/configuration/client#host_volume-stanza) provide a fine tune
...
- The client host's environment variables are now cleaned before rendering the
  ...
  [`env`](/nomad/docs/job-specification/env) stanza to allow access to the variable in
  the template.
...
[`to`](/nomad/docs/job-specification/network#to) field on group network port stanzas
will be interpolated properly. Please see the
[`network`](/nomad/docs/job-specification/network/) stanza documentation for details.
